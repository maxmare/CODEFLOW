...
# raster draw a multi-face polygon in 2d.
#-----------------------------------------------------------------------
...
def sort2d(lista):
	...
	while( unsorted ):
		...
		for i, item in enumerate(lista):
			if i < len(lista)-1:
				#print i, len(lista)
				if int(lista[i][1]) > int(lista[i+1][1]):
					temp = lista[i+1]
					lista[i+1] = lista[i]
					lista[i] = temp
					unsorted = True
				elif int(lista[i][1]) == int(lista[i+1][1]):# same row
...
points = []
#'''
for i in range( int( ( random.random() + 0.0 ) * 10  )+3 ):
	a = ( random.random() + 0.0 ) * 100
	b = ( random.random() + 0.0 ) * 100
	lines.append( [ a, b ] )
#'''
'''
lines.append( [ 30.0 , 25.0  ] )# 0
lines.append( [ 100.0, 100.0 ] )# 1 
lines.append( [ 100.0, 132.0 ] )# 2
lines.append( [ 88.0 , 132.0 ] )# 3
lines.append( [ 88.0 , 121.0 ] )# 4
lines.append( [ 68.0 , 122.0 ] )# 5
lines.append( [ 68.0 , 75.0 ] )# 6
lines.append( [ 18.0 , 114.0 ] )# 7
lines.append( [ 17.0 , 95.0  ] )# 8
#'''
'''
lines = []
lines.append( [ 31.92 , 48.49  ] )# 0
lines.append( [ 35.65, 49.66 ] )# 1 
lines.append( [ 71.94, 86.77 ] )# 2
lines.append( [ 59.99, 49.67 ] )# 3
lines.append( [ 76.69, 41.00 ] )# 4
#'''
'''
lines = []
lines.append( [ 39.86, 35.80 ] )# 0
lines.append( [ 08.07, 21.35 ] )# 1 
lines.append( [ 27.83, 47.39 ] )# 2
lines.append( [ 10.35, 95.24 ] )# 3
lines.append( [ 95.57, 71.94 ] )# 4
lines.append( [ 88.14, 51.79 ] )# 2
lines.append( [ 96.91, 47.33 ] )# 3
lines.append( [ 91.51, 28.57 ] )# 4
lines.append( [ 54.16, 39.70 ] )# 4
#'''
'''
lines = []
lines.append( [48.340445945856523, 3.1504815177300638] )
lines.append( [44.340493512056547, 17.555036568808003]   )
lines.append( [14.741116531584664, 29.072599670416778]     )
lines.append( [37.306068730107178, 53.061664001451206]       )
lines.append( [0.48616115736418442, 97.168077163495198]    )
lines.append( [53.950356318769344, 55.508801223831775]   )
lines.append( [72.460208057496018, 80.085946459765722] )
#'''
'''
lines = []
lines.append( [31.577771855520453, 6.4221367350566165] )
lines.append( [16.402595844081169, 14.219400929707238] )
lines.append( [6.8670888929837854, 70.583003531795768] )
lines.append( [37.862468163312144, 57.342328500065506] )
lines.append( [71.957262295832209, 96.363092992059663] )
lines.append( [74.035246046807217, 11.430341718465542] )
#'''
'''
lines = []
lines.append( [45.98213811448796, 37.849674869987261] )
lines.append( [24.002454603728797, 5.9765341121476911] )
lines.append( [8.3981703593123509, 49.208688036946754] )
lines.append( [18.414920422275305, 68.390911733358053] )
lines.append( [76.80012941397419, 61.515462292203679] )
lines.append( [79.905346556576603, 53.297101106776566] )
lines.append( [53.581199698000972, 38.901489511427137] )
lines.append( [57.647787006132859, 6.1130086851644556] )
#'''

print 'lines: '
for line in lines:
	print line
lines = sorted(lines, key=sortByAngle)
print 'lines: '
for line in lines:
	print line
...
for i in range( len( lines ) ):
	...
	if j > ( len( lines ) - 1):
		j = 0
	segments.append( (lines[i], lines[j]) )
...
# draw, store all the segments,
for segment in segments:
	#biggest dimension in pixels,
	r = 125; g = 0; b = 0; 
	newColor = (r, g, b)
	horEdge = abs( segment[0][0] - segment[1][0] )
	longEdge = horEdge
	verEdge = abs( segment[0][1] - segment[1][1] )
	if verEdge > horEdge:
		longEdge = verEdge
		horEdge = 0
	else:
		verEdge = 0
	delta = ( (segment[1][0] - segment[0][0] ) / longEdge, (segment[1][1] - segment[0][1] ) / longEdge)
	x = segment[0][0]
	y = segment[0][1]
	for i in range( int( longEdge )+1 ):
		d.point( ( x, y ), fill = newColor )
		if horEdge != 0:
			#d.point( ( x, y-1 ), fill = newColor )
			#d.point( ( x, y+1 ), fill = newColor )
			pass
		else:
			#d.point( ( x-1, y ), fill = newColor )
			#d.point( ( x+1, y ), fill = newColor )
			pass
		edgeList.append( [x,y] )
		x += delta[0]
...
print '________edgeList:'
for p in edgeList:
	print p

sortedList = sort2d(edgeList)
print '________sortedList: '
for p in sortedList:
	print p

tempLeft = 0; tempRight = 0; row = 0
point = []
points = []
prevPoint = 0

# get a point list for filling in,
...
print '________points: '
for p in points:
	print p

# draw the face,
row = int( sortedList[0][1] )
for point in points:
	start = 0; end = 0; amount = 0; prevAmount = 0; col = 0; count = 0
	diffVal = int(point[0])
	diffCount = 1
	for p in point:
		if abs(int(p)-diffVal) > 1:
			diffCount += 1
		diffVal = int(p)
	print point, diffCount
	start = point[0]
	end = point[-1]
	for j in range( start, end ):
		pick = txt.getpixel( ( j, row ) )
		newColor = (pick[0], pick[1]+50, pick[2]+150)
		pickb = txt.getpixel( ( j, row-1 ) )
		if (pickb[0]+pickb[1]+pickb[2]) == 0:
			newColor = (pick[0], pick[1]+150, pick[2]+50)
			d.point( ( j, row ), fill = newColor )
		else:
			d.point( ( j, row ), fill = newColor )
	row += 1

fill = False; prevFill = False
drawing = False
count = 0
no = 0
