...
point findIntersection(line l1, line l2, point pt){
	point p1 = {0,0}; 
	point pInt = {0,0};
	if( math.abs(l1[1]-l1[1]) > math.abs(l1[0]-l1[0]) ){// di stands for direction index
		int di = 1; int dj = 0;
		}
	else{ 
		int di = 0; int dj = 1;
		}
	point pivot = [ l1[di+2], l1[dj+2] ];
	float ppu = ( l1[di+2] - l1[di] ) / ( l1[dj+2] - l1[dj] );# projection per unit
	float line2b = { l2[di], l2[dj], l2[di+2], l2[dj+2] };
